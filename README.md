...
**529 tests** pass in the dependency-free default run and **976** with
`DATABASE_URL` set, because the database-backed files stop being skipped and the
dual-backend suites run their Postgres half. (The line previously read 385 and
687; the counts are updated rather than removed, and the earlier pair is kept in
this parenthesis so the growth stays auditable.)
